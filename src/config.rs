...
impl Config {
    pub fn from_env() -> Self {
        ...
        let dry_run = env::var("DRY_RUN").map(|v| v == "true" || v == "1").unwrap_or(true);
        let btc_only = env::var("BTC_ONLY").map(|v| v == "true" || v == "1").unwrap_or(false);
        let crypto_only = env::var("CRYPTO_ONLY").map(|v| v != "false").unwrap_or(true);
        ...
        let combined_max_price = env::var("COMBINED_MAX_PRICE").ok().and_then(|v| v.parse().ok()).unwrap_or(1.0);
        let order_count = env::var("ORDER_COUNT").ok().and_then(|v| v.parse().ok()).unwrap_or(1);
        let check_exchange = env::var("CHECK_EXCHANGE").map(|v| v != "false").unwrap_or(true);
        let time_in_force = env::var("TIME_IN_FORCE").unwrap_or_else(|_| "fill_or_kill".to_string());
        let discover_btc_events = env::var("DISCOVER_BTC_EVENTS").map(|v| v != "false").unwrap_or(true);
        let discover_series = env::var("DISCOVER_SERIES").map(|v| v != "false").unwrap_or(false);
        let series_category = env::var("SERIES_CATEGORY").unwrap_or_else(|_| "crypto".to_string());
        let series_frequency = env::var("SERIES_FREQUENCY").unwrap_or_else(|_| "fifteen_min".to_string());
        let events_limit = env::var("EVENTS_LIMIT").ok().and_then(|v| v.parse().ok()).unwrap_or(200);
        let log_decisions = env::var("LOG_DECISIONS").map(|v| v == "true" || v == "1").unwrap_or(false);
...
fn split_base_url(raw: &str) -> (String, String) {
    ...
    (raw.trim_end_matches('/').to_string(), "/trade-api/v2".to_string())
}
